...
<html>
  ...
  <body>
    <div class="iphone-13-pro-max-7">
      ...
      <div class="iphone-13-pro-max-7-child7"></div>
      ...
      <select class="tmtype">
      <option class="tmtype" id="">TM Type</option>
      <?php   
        $query = "SELECT DISTINCT(Tmtype) FROM cancerdetails";
        $exequery = mysqli_query($conn, $query);
        while($value = mysqli_fetch_assoc($exequery)) { ?>
      <option class="tmtype" value=""><?php echo $value['Tmtype']; ?></option>
      <?php } ?>
       </select>
      <select class="ntype">
      ...
      <?php   
        ...
        $exequery = mysqli_query($conn, $query);
        while($value = mysqli_fetch_assoc($exequery)) { ?>
      <option class="ntype" value=""><?php echo $value['Ntype']; ?></option>
      <?php } ?>
       </select>
      <div class="cstage">Cancer Stage </div>
      <div class="survyear">Survival Year Calc</div>
      <div class="survdata">( Last 5 or 10 Yrs data)</div>
      <div class="survrate">Survival Rate</div>
      <div class="linksrc">Link Source</div>
      <div class="treating-doctors">Treating Doctors</div>
